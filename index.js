import{a as m,S,N as q,P as _,K as x,A as T,b as $}from"./assets/vendor-Dal2vjfK.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const d of a.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&n(d)}).observe(document,{childList:!0,subtree:!0});function o(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function n(s){if(s.ep)return;s.ep=!0;const a=o(s);fetch(s.href,a)}})();const I=document.querySelector("[data-menu-open]"),O=document.querySelector("[data-menu-close]"),L=document.querySelector("[data-menu]");I.addEventListener("click",()=>{L.classList.add("is-open"),document.body.style.overflow="hidden"});O.addEventListener("click",()=>{L.classList.remove("is-open"),document.body.style.overflow=""});document.querySelectorAll(".nav-link-mob").forEach(e=>e.addEventListener("click",()=>{L.classList.remove("is-open"),document.body.style.overflow=""}));const r={dropdownToggle:document.querySelector(".dropdown-toggle"),dropdownMenu:document.querySelector(".dropdown-menu"),categoriesList:document.querySelector(".categories-list"),bookCardList:document.querySelector(".books-cards-list"),showMoreBtn:document.querySelector(".show-more-btn"),bookModalBackdrop:document.querySelector("#book-modal-backdrop"),bookModalContent:document.querySelector(".book-modal-content"),bookModalClose:document.querySelector("#book-modal-close")};m.defaults.baseURL="https://books-backend.p.goit.global";function y(e,t){throw console.error(`${t} error:`,e.message||e),e}async function N(){try{return(await m.get("/books/category-list")).data.filter(o=>o.list_name&&o.list_name.trim()!=="").map(o=>o.list_name)}catch(e){y(e,"getCategoryList")}}async function V(){try{return(await m.get("/books/top-books")).data.flatMap(o=>o.books)}catch(e){y(e,"getTopBooks")}}async function D(e){try{const{data:t}=await m.get("/books/category",{params:{category:e}});return console.log(t),t}catch(t){y(t,"getBooksByCategory")}}async function H(e){try{const{data:t}=await m.get(`/books/${e}`);return t}catch(t){y(t,"getBookById")}}function R(e){const o=["All categories",...e].map(n=>`<li class="dropdown-item">${n}</li>`).join("");r.dropdownMenu.innerHTML=o}function j(e){const o=["All categories",...e].map(n=>`<li class="categories-item" data-category="${n}">${n}</li>`).join("");r.categoriesList.innerHTML=o}function K(e){return e?e.toLowerCase().split(" ").map(t=>t.charAt(0).toUpperCase()+t.slice(1)).join(" "):""}function U(e){const t=e.map(({_id:o,book_image:n,description:s="description || title",title:a,author:d,price:i})=>{const l=K(a),v=parseInt(i);return`<li class="books-card" data-id="${o}">
  ...
  `,o.appendChild(n);const s=n.querySelector(".dots"),a=n.querySelector(".prev"),d=n.querySelector(".next");let i=0,l=1;function v(){s.innerHTML="";const c=l===1?t.length:t.length-l+1;for(let u=0;u<c;u++){const p=document.createElement("span");p.className="dot"+(u===0?" active":""),s.appendChild(p)}}function h(c){e.style.transition="opacity 0.3s ease",e.style.opacity="0",setTimeout(()=>{c(),e.style.opacity="1"},300)}function g(){const c=t.length;t.forEach((p,b)=>{p.style.display=b>=i&&b<i+l?"flex":"none"}),s.querySelectorAll(".dot").forEach((p,b)=>p.classList.toggle("active",b===i)),a.classList.toggle("is-disabled",i===0),d.classList.toggle("is-disabled",i>=c-l)}function E(){const c=window.innerWidth;c>=1440?(l=t.length,t.forEach(u=>u.style.display="flex"),n.style.display="none",e.style.transition="",e.style.opacity="1"):(l=c<768?1:2,n.style.display="flex",i=0,v(),g())}d.addEventListener("click",()=>{const c=t.length;i<c-l&&h(()=>{i++,g()})}),a.addEventListener("click",()=>{i>0&&h(()=>{i--,g()})}),s.addEventListener("click",c=>{if(!c.target.classList.contains("dot"))return;const u=Array.from(s.children).indexOf(c.target);h(()=>{i=u,g()})}),window.addEventListener("resize",E),E()});
